...
public class StatsUtils {
    ...
    // Some internal configuration params.
    ...
    // Uses the dynamic box cox transform to try to transform the data
    // in order to follow the normal distirbution.
    ...
    private static Float getBoxCoxEstimate(Float[] data) {
        ...
        for (double cut = 1.0; cut > 0; cut -= 0.1) {
            ...
            while (lambda <= 3.0) {
                ...
                resultBoxCox = StatsUtils.swilk(boxCoxData);
                resultOriginal = StatsUtils.swilk(rawData);
                if (resultBoxCox != null) {
    ...
    // Uses the bootstrap to provide confidence intervals of the mean
    // and standard deviation of the data.
    private static Float[] getConfidenceIntervals(Float[] data) {
        int dataSize = data.length;
        // This is the minimum number of subsamples
        // to get meaningful statistical significance.
        ...
        // Iterate until the distribution is normal.
        ...
        while (((resultMean != null && resultMean[1] < alpha) &&
            ...
            for (int i = 0; i < numIterations; i++) {
                Float[] tmpSample = getRandomSample(data, sampleSize, dataSize, random);
                Float mean = getMean(tmpSample);
                Float sd = getSD(tmpSample, mean);
                distrMeans.add((double) mean);
                distrSd.add((double) sd);
            }
            
            // Varify that the resulting distribution is a normal one.
            resultMean = StatsUtils.swilk(distrMeans);
            resultSd = StatsUtils.swilk(distrSd);
            
            sampleSize += dataSize;
            curNumIterations++;
        }
        
        Float[] meanArray = new Float[distrMeans.size()];
        Float[] sdArray = new Float[distrSd.size()];
        
        for (int i = 0; i < distrMeans.size(); i++) {
            meanArray[i] = new Float(distrMeans.get(i));
        }
        for (int i = 0; i < distrSd.size(); i++) {
            sdArray[i] = new Float(distrSd.get(i));
        }
        
        Float meanMu = getMean(meanArray);
        Float sdMu = getSD(meanArray, meanMu);
        Float meanSd = getMean(sdArray);
        Float sdSd = getSD(sdArray, meanSd);
        
        // Returns the confidence intervals
        // of the mean and of the standard deviation.
        return new Float[]{sdMu, sdSd};
    }
    
    // Computes a sample X.
    private static Float[] getRandomSample(Float[] data, int sampleSize, int dataSize, Random rand) {
        Float[] tmp = new Float[sampleSize];
        for (int i = 0; i < sampleSize; i++) {
            tmp[i] = data[rand.nextInt(dataSize)];
        }
        return tmp;
    }
